<?php # coding: utf-8
   ...
   { // Formatage de page
      ...
      $CONTENT = preg_replace('/^"""(.*)(\n)/Um', "<blockquote><small>Citation</small>$1</blockquote>$2", $CONTENT);
      $CONTENT = preg_replace('/^@@(.*)(\n)/Um', "<div class='warning'>$1</div>$2", $CONTENT);
      ...
      function ascendant($counter,$level){
         $return = '';
         for($i=1;$i<$level;$i++){
            $return.=($counter[$i]-1).'.';
         }
      ...
      function name_title($matches) // replace titles
         ...
         return '<h'.$titres[$i][0].'><a class="nolink" name="'.$titres[$i][1].'">'.$titres[$i][2].'</a></h'.$titres[$i][0].'>';}
      ...
      if (strpos($CONTENT,'%TOC%') !== FALSE)
      {
         ...
         $toc = '<div id="toc"><h4>Sommaire</h4><ul>';
         $counter = array('1','1','1','1','1');
         for ($i=0;$i<$nbAncres;$i++) {

          

            $toc .= '<li style="margin-left:'.($titres[$i][0]*10).'px;" class="toc-h'.$titres[$i][0].'">';
            $toc .= '<a  href="#'.urlencode($titres[$i][1]).'">'.ascendant($counter,$titres[$i][0]).$counter[$titres[$i][0]].'.'.preg_replace('#[\[\]]#','',$titres[$i][2]).'</a>';
            $toc .= '</li>';
            
            $counter[$titres[$i][0]]=$counter[$titres[$i][0]]+1;
